...
export default function Add({open, handleClose, setMiracle }) {
    ...
    const handleSubmit = async () => {
        if(inputs.title !=='' && inputs.descrpition !== '' && inputs.event_date ){
        try{
            const result = await FetchAPI.insertMiracle({
                title : inputs.title,
                descrpition : inputs.descrpition, 
                event_date: getDate(inputs.event_date)
            });
            
            setMiracle(result.miracleList);
            handleClose(false);
        }catch(e){
            console.log('Error...',e);
        }
