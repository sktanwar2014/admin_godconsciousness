 ...
 console.log(props)
  ...
  useEffect(() => {
    if(Object.keys(props)[2] === 'data'){
      setInputs({id : props.data.id,title: props.data.title , description: props.data.description})
    }
  },[])
  ...
  const handleSubmit = async () => {
    if(inputs.id !== '',inputs.description !=='' && inputs.description !== ''){
      try{
        
        const response = await FetchAPI.updateDirection({id:inputs.id,  title: inputs.title, description: inputs.description});
  ...
  async function submitHandle(e) {
    ...
    const URL = `http://localhost:5000/api/updateDirection`;
    try {
      const { data } = await axios(URL, Object.assign({}, PARAMS({ methodType: 'POST' }), {
        data: { id: inputs.id,title: inputs.title, description: inputs.description},
      }),
    );
      return data;
    } catch (error) {
