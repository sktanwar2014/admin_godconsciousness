...
export default function Add({open, handleClose, setDirection }) {
    ...
    const handleSubmit = async () => {
        if(inputs.title !=='' && inputs.descrpition !== ''  ){
        try{
            const result = await FetchAPI.insertDirection({
                title : inputs.title,
                description : inputs.description, 
                //event_date: getDate(inputs.event_date)
            });
            
            setDirection(result.directionList);
            handleClose(false);
        }catch(e){
            console.log('Error...',e);
        }
