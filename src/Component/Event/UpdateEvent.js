...
export default function EventEditor(mainProps) {
 ...
 console.log(props)
  ...
  const [inputs, setInputs] = useState({title:'', description:'', posted_by: '',posted_on: ''});  
  ...
  useEffect(() => {
    if(Object.keys(props)[2] === 'data'){
      setInputs({title: props.data.title , description: props.data.description,posted_by: props.data.posted_by, posted_on: props.data.posted_on })
    }
  },[])
  ...
  const handleSubmit = async () => {
    if(inputs.title !=='' && inputs.description !== '' && inputs.posted_by !=='' && inputs.posted_on ){
      try{
        
        const response = await FetchAPI.updateEvent({  title: inputs.title, description: inputs.description, posted_by: inputs.posted_by, posted_on: inputs.posted_on });
  ...
  async function submitHandle(e) {
    ...
    try {
      const { data } = await axios(URL, Object.assign({}, PARAMS({ methodType: 'POST' }), {
        data: { title: inputs.title, description: inputs.description, posted_by: inputs.posted_by, posted_on: inputs.posted_on},
      }),
    );
      return data;
    } catch (error) {
