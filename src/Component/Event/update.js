...
export default function Update({open, handleClose, rowData, setEvents }) {
    ...
    const handleSubmit = async () => {
        if(inputs.id !== '',inputs.title !=='' && inputs.description !== '' && inputs.event_date ){
        try{
            const result = await FetchAPI.updateEvent({
                id: inputs.id,
                title : inputs.title,
